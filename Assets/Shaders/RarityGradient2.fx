...
float uMinimumDarknessValue;        // So more noises are viable to use, since some are dark and make the text hard to read
float uNoiseBrightnessMultiplier;   // Same for this one, just more options in case the above isn't appealing on the noise
float time;
...
float4 PixelShaderFunction(float2 coords : TEXCOORD0, float4 screenPosition : vPos) : COLOR0
{
    ...
    float2 rotatedCoords = RotateCoords(screenPosition - uUIPosition, uAngle);
    ...
    // Convert HSL to RGB
    float3 HSL = float3(uBaseHSL.x, saturation, uBaseHSL.z);
    ...
    if (uUseNoise) {
        ...
        noiseColor.rgb = clamp(noiseColor.rgb, float3(uMinimumDarknessValue, uMinimumDarknessValue, uMinimumDarknessValue), float3(10.0, 10.0, 10.0));
        color.rgb *= noiseColor.rgb * uNoiseBrightnessMultiplier;
    }
