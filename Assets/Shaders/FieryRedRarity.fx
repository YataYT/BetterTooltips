...
float4 PixelShaderFunction(float2 coords : TEXCOORD0, float4 screenPosition : vPos) : COLOR0
{
    ...
    float3 fieryColor = float3(1.0, 0.5 + 0.5 * cos((time * uTimeMultiplier) + (pos.x * uAreaMultiplier)), 0.0) + float3(1.0, 0.5 + 0.5 * cos((time * uTimeMultiplier) + (pos.y * uAreaMultiplier)), 0.0);
    color.rgb *= fieryColor;

    float alpha = uGlow ? 0.0 : color.a;
    float fade = uGlow ? GetFadeOutValue(newCoords) * 2.0 : 1.0;

    return float4(color.rgb * fade / 2.0, alpha);
}
